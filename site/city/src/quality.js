...
//
// Third copy of this file, and the third time only `apply()` and the per-preset
// scene flags differed while the scaler itself came across untouched. That is now
// a pattern rather than a coincidence, and it is written up in ../../README.md:
// the shared version wants an injected onApply(preset) plus a per-game extras
// bag, and there are finally three consumers to shape it around.
...
export class Quality {
	...
	apply() {
		...
		g.city.setShadows(p.shadows, p.shadowSize)
		// The lamp posts are ~64 little meshes; the neon bands are what the game
		// looks like, so they survive to the bottom tier.
		g.city.setDecor(p.lamps, p.neon)
